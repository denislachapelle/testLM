...
/// using DSmoother on the diagonals of A, S1, S2.
...
private:
   ...
   const SparseMatrix &M1;
   const SparseMatrix &M2;

   // We allocate these after we’ve built the diagonal vectors.
   DSmoother *precA;
   DSmoother *precM1;
   DSmoother *precM2;
...
MFEM_VERIFY(M2.Finalized(), "Matrix M2 must be finalized.");
      ...
      // --- Build diagonals and DSmoothers (inside the class) ---
         precA->SetOperator(A);   // this is your "new DSmoother(A->GetDiag())"
         precM1->SetOperator(M1);
         precM2->SetOperator(M2); // "new DSmoother(S2->GetDiag())"
   }
...
ThreeBlockDiagonalPreconditioner prec(BlockOffsets, a.SpMat(), MBLF_LM1->SpMat(), MBLF_LM2->SpMat());
